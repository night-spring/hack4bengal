...
useEffect(() => {
  ...
  const navigationItems = [
    ...
    { id: 'resources', label: 'Resources', icon: FiDatabase, url: "" },
    { id: 'prediction', label: 'AI Predictions', icon: FiTrendingUp, url: "" },
    { id: 'alerts', label: 'Alerts', icon: FiAlertTriangle, badge: alerts.filter(a => a.type === 'critical').length, url: "" },
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <motion.div
        ...
        className={`fixed inset-y-0 left-0 z-50 w-64 shadow-lg transform lg:hidden ${darkMode ? 'bg-gray-800 border-r border-gray-700' : 'bg-white border-r border-gray-200'
          }`}
      >
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center w-full px-4 py-3 text-left rounded-lg transition-colors duration-200 ${activeTab === item.id
                    ? darkMode
          ...
          <div className={`absolute bottom-0 w-full ${darkMode ? 'border-t border-gray-700' : 'border-t border-gray-200'}`}>
            <div className="px-2 py-2 space-y-1">
              ...
              <button
                ...
                className={`flex items-center w-full px-4 py-3 text-left rounded-lg transition-colors duration-200 ${activeTab === 'profile'
                  ? darkMode
                    ? 'bg-cyan-900 text-cyan-100'
                    : 'bg-cyan-50 text-cyan-600'
                  : darkMode
        ...
        <div className="px-6 h-16">
          <div className="flex items-center justify-between h-full">
              ...
              {navigationItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ${activeTab === item.id
                      ? darkMode
                        ? 'bg-cyan-900 text-cyan-100'
                        : 'bg-cyan-50 text-cyan-600'
                      : darkMode
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                onClick={() => navigateTo('profile')}
                className={`flex items-center space-x-2 px-3 py-2 rounded-lg transition-colors duration-200 ${activeTab === 'profile'
                  ? darkMode
                    ? 'bg-cyan-900 text-cyan-100'
                    : 'bg-cyan-50 text-cyan-600'
                  : darkMode
